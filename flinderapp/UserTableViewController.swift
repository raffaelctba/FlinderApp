...
class UserTableViewController: UITableViewController, NSFetchedResultsControllerDelegate, UserDelegate {
    ...
    override func viewDidLoad() {
        ...
        let client = MSClient(applicationURLString: "https://flinderapp.azurewebsites.net")
        let managedObjectContext = (UIApplication.sharedApplication().delegate as! AppDelegate).managedObjectContext!
        self.store = MSCoreDataStore(managedObjectContext: managedObjectContext)
        client.syncContext = MSSyncContext(delegate: nil, dataSource: self.store, callback: nil)
        self.table = client.syncTableWithName("User")
    ...
    func configureCell(cell: UITableViewCell, indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if let text = item.valueForKey("first_name") as? String {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject!)
    {
        if segue.identifier == "addUser" {
            let userController = segue.destinationViewController as! UserViewController
            userController.delegate = self
        }else{
            print("not addUser segue");
        }
    }
    ...
    func didSaveItem(text: String)
    {
        ...
        let userToInsert = ["first_name": text, "__createdAt": NSDate()]
        
        UIApplication.sharedApplication().networkActivityIndicatorVisible = true
        self.table!.insert(userToInsert) {
            (item, error) in
            UIApplication.sharedApplication().networkActivityIndicatorVisible = false
            if error != nil {
                print("Error: " + error!.description)
